...
var userData = {
    add: function(req, res, next) {
        pool.getConnection(function(err, connection) {
            ...
            connection.query(sql.insert, [param.phone, param.pass], function(err, result) {
                if (result) {
                    result = 'add'
                }
    ...
    update: function(req, res, next) {
        var param = req.body;
        if (param.phone == null || param.pass == null || param.id == null) {
            json(res, undefined);
            return;
        }
        pool.getConnection(function(err, connection) {
            connection.query(sql.update, [param.phone, param.pass, +param.id], function(err, result) {
                if (result.affectedRows > 0) {
                    result = 'update'
                } else {
                    result = undefined;
